    ...
    this.uploader = function(op) {
        ...
        var getUpToken = function() {
            if (!op.uptoken) {
                ...
                if(cache.loading === false && cache.token) {
                    that.token = cache.token;
                    return;
                ...
                ajax.onreadystatechange = function() {
                    if (ajax.readyState === 4 && ajax.status === 200) {
                        ...
                        that.token = (res.obj.token);

                        if(cache) {
                            cache.loading = false;
                        }

                        if(that.token === 'string') {
                            if(cache) {
                                cache.token = that.token;
