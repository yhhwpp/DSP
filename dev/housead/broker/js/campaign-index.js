    ...
    CampaignList.prototype = {
        ...
        _fnCustomColumn: function (td, sData, oData, row, col, table) { // 自定义列表显示
            ...
            if (thisCol === 'products_name') {
                ...
                if (oData.ad_type === CONSTANT.ad_type_other || oData.revenue_type === CONSTANT.revenue_type_cps || oData.revenue_type === CONSTANT.revenue_type_cpa) {
                    $(td).html('-');
                }
                else {
        ...
        _fnInitComplete: function (settings, json, obj) {
            ...
            if ($('select[data-name=ad_type]') && $('select[data-name=ad_type]').length > 0) { // 获取广告类型
                var _adType = {};
                _adType[CONSTANT.ad_type_app_market] = LANG.ad_type[CONSTANT.ad_type_app_market];
                self._fnCreateSelect($('select[data-name=ad_type]'), _adType);
            }
            if ($('select[data-name=platform]') && $('select[data-name=platform]').length > 0) { // 获取平台
                $.get(API_URL.trafficker_common_platform, function (response) {
                    if (response && response.res === 0 && response.obj && response.obj.length > 0) {
                        var options = '';
                        for (var pi = 0, pj = response.obj.length; pi < pj; pi++) {
                            options += '<option value="' + response.obj[pi] + '">' + LANG.platform_group[response.obj[pi]] + '</option>';
                        }
            ...
            if ($('select[data-name=status]') && $('select[data-name=status]').length > 0) { // 获取状态
                var _status = LANG.campaign_status_select;
                delete _status[4];
                self._fnCreateSelect($('select[data-name=status]'), _status);
            }
        },
        _fnFilter: function () {
            var oFilter = {
