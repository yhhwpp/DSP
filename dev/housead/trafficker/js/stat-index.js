...
var Stat = (function ($) {
    ...
    Stat.prototype = {
            ...
            $.post(API_URL.common_affiliate_revenue_type, function (result) {
                if (!result.res) {
                    ...
                    if (result.obj && $.isArray(result.obj.revenue_type)) {
                        var revenue_type = 0;
                        if ($.inArray(CONSTANT.revenue_type_cpa, result.obj.revenue_type) > -1) {
                            _this.itemList.sum_cpa.hidden = false;
                            _this.itemList.cpa_cpd.hidden = false;
                            $('#import-cpa').show();
                            revenue_type += CONSTANT.revenue_type_cpa;
                        }
                        if ($.inArray(CONSTANT.revenue_type_cps, result.obj.revenue_type) > -1) {
                            $('#import-cps').show();
                            revenue_type += CONSTANT.revenue_type_cps;
                        }
                        if (revenue_type === CONSTANT.revenue_type_cps) {
                            $('#export-muban-cps').show();
                        }
                        else if (revenue_type === CONSTANT.revenue_type_cpa) {
                            $('#export-muban-cpa').show();
                        }
                        else if (revenue_type === (CONSTANT.revenue_type_cpa + CONSTANT.revenue_type_cps)) {
                            $('#export-muban-all').show();
                        }
                    }
        ...
        _getGeneralData: function () {
            ...
            this._ajaxOpen();
            $.get(API_URL.trafficker_stat_self_zone, param, function (result) {
                if (!result.res) {
                    _this._ajaxClose();
        ...
        fnInit: function (dayType, id, name) {
            ...
            $('#export-report-btn').click(function () {
                var url = API_URL.trafficker_stat_self_zone_excel
